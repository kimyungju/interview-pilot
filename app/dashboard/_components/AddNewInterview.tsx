...
export default function AddNewInterview() {
  ...
  const [bankSelection, setBankSelection] = useState<"random" | "in-order">("random");
  ...
  const handleSuggestQuestions = async () => {
    ...
    try {
      ...
      setQuestionBank(questions.join("\n"));
    } catch (error) {
    ...
    try {
      ...
      const { mockId } = await createInterview(
        ...
        mode === "auto" ? jobExperience : "",
        {
          ...
          questionCount,
          language,
          customQuestions: bankLines.length > 0 ? bankLines : undefined,
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-medium">{t("create.questionBank")}</label>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                    onClick={handleSuggestQuestions}
                    disabled={loadingSuggestions || !jobPosition}
                  >
                    {loadingSuggestions ? (
                      <><Loader className="animate-spin mr-1 h-3 w-3" /> {t("create.suggesting")}</>
                    ) : (
                      <><Sparkles className="mr-1 h-3 w-3" /> {t("create.suggestQuestions")}</>
                    )}
                  </Button>
                </div>
                <Textarea
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  {t("create.questionBankHint")}
                </p>

                {questionBank.trim() && (
                  <div className="mt-3 flex items-center gap-4">
                    <div className="flex rounded-lg border-2 border-border overflow-hidden">
                      {(["random", "in-order"] as const).map((m) => (
                        <button
                          key={m}
                          type="button"
                          onClick={() => setBankSelection(m)}
